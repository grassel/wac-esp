...
* Function imports are unsupported, no dynamic DLL loading, no calls to the OS.
  I might add some support in the future.
* Systematic testing has not been done.
...
Run once, the only thing to do is to adjust the device port of your module:
```make menuconfig``` 
...
The output is pretty verbose because tracing is enabled (change in platform.h). If everything goes fine it prints the return value, tells its done and reboots to start over. Stop the monitor with ```CTRL - ]```.
